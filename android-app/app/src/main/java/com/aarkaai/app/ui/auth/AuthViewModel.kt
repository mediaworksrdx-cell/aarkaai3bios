...
class AuthViewModel(application: Application) : AndroidViewModel(application) {
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            if (storedToken != null) {
                ...
                // No stored token — allow user to see AuthScreen on first launch
                uiState = uiState.copy(
                    isLoggedIn = false,
                    isCheckingToken = false
                )
    ...
    private suspend fun loginOrRegisterGuest() {
        ...
        try {
            val res = RetrofitClient.api.getVisitorToken()
            tokenManager.saveAuth(res.access_token, res.user_id, res.name ?: "Web Visitor")
            uiState = uiState.copy(
                isLoading = false,
                token = res.access_token,
                userId = res.user_id,
                userName = res.name ?: "Web Visitor",
                isLoggedIn = true,
                isCheckingToken = false,
                error = null
            )
        } catch (ex: Exception) {
            // Offline / timeout fallback: provide immediate guest access so app never hangs
            val offlineToken = "offline_guest_" + java.util.UUID.randomUUID().toString()
            tokenManager.saveAuth(offlineToken, "guest_user", "Web Visitor")
            uiState = uiState.copy(
                isLoading = false,
                token = offlineToken,
                userId = "guest_user",
