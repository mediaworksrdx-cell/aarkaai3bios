...
def download_file(
    ...
    Files are retrieved from the sandboxed SAFE_WORK_DIR. Requires authentication.
    """
    import mimetypes
    ...
    # Prevent path traversal attacks by extracting strictly the basename
    ...
    # Check user-namespaced directory first, then root workspace
    user_file_path = (safe_dir / current_user.id / clean_filename).resolve()
    shared_file_path = (safe_dir / clean_filename).resolve()

    file_path = None
    if user_file_path.is_file():
        file_path = user_file_path
    elif shared_file_path.is_file():
    ...
    media_type, _ = mimetypes.guess_type(clean_filename)
    return FileResponse(
        path=str(file_path),
...
async def screener_endpoint(
    ...
    """Full institutional-grade multi-factor stock screening. Requires authentication."""
...
async def list_strategies(
    ...
    """List all 20 pre-built screening strategies with metadata. Requires authentication."""
    if not _screener_endpoints_available or not _screener_api_agent:
        raise HTTPException(status_code=503, detail="Screener agent not available")
...
async def get_market_regime(
    ...
    """Get current market regime classification using dynamic indicators. Requires authentication."""
...
async def get_sector_rankings(
    ...
    """Get all available sectors and their stock counts. Requires authentication."""
    if not _screener_endpoints_available or not _screener_api_agent:
        raise HTTPException(status_code=503, detail="Screener agent not available")
...
async def get_screener_provenance(
    ...
    or real-time provenance audit trail for enterprise compliance. Requires authentication."""
    try:
