...
async def stream_query(query: str, user_id: str = "default", session_id: str = "default", mode: str = "production", model_override: str | None = None):
    ...
    if model_override and (model_override.startswith("gemini") or model_override.startswith("claude") or model_override.startswith("gpt")):
        # Fetch web context if web search is needed
        web_context = ""
        try:
            from modules.intent_detector import detect_intent
            intent_info = detect_intent(query)
            if intent_info.get("primary_intent") in ("web_search", "realtime", "news") or "latest" in query.lower() or "news" in query.lower() or "search" in query.lower():
                yield {"type": "status", "status": "Searching Google for live results..."}
                from modules.web_search import get_web_context
                web_context = get_web_context(query, max_results=5)
        except Exception as exc:
    ...
    if _is_pdf_generation_query(query):
        ...
        if coder_result and coder_result.get("code"):
            ...
            # Coder pipeline failed — fall back to direct 7B streaming
            logger.warning("Aarka Coder returned no code — falling back to direct 7B generation")
        ...
        # Stream tokens live token-by-token directly to the client as they are generated by the 7B model
        import asyncio
        for token in aarkaa_engine.stream_final_response(query, fused_context, intent=intent, lang=detected_lang, mode=mode, history=chat_ctx, user_facts=user_facts):
            full_response += token
            yield {"type": "content", "token": token}
