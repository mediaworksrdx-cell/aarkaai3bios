...
def sanitize_python_script(content: str) -> str:
    """Sanitizes Python code strings generated by LLMs.
    
    Strips markdown code fences, triple quotes, unescapes newlines,
    trims stray leading/trailing quotes, and auto-inserts missing standard imports.
    """
    ...
    # Handle literal string escapes if string was double-escaped
    if "\\n" in cleaned and "\n" not in cleaned:
    ...
    # Strip outer triple quotes: """...""" or '''...'''
    if (cleaned.startswith('"""') and cleaned.endswith('"""')) or (cleaned.startswith("'''") and cleaned.endswith("'''")):
        cleaned = cleaned[3:-3].strip()

    # Strip leading stray quotes from LLM formatting slips
    ...
    # Strip trailing stray quotes from LLM formatting slips
    ...
    # Re-check markdown fences in case code fences were inside outer quotes
    cleaned = re.sub(r'^```(?:python|py)?\s*\n?', '', cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r'\n?```\s*$', '', cleaned).strip()

    # Ensure missing standard imports for common disk / system utilities
    lines = cleaned.splitlines()
    has_os = any(re.match(r'^\s*(import\s+os\b|from\s+os\b)', l) for l in lines)
    has_shutil = any(re.match(r'^\s*(import\s+shutil\b|from\s+shutil\b)', l) for l in lines)
    has_sys = any(re.match(r'^\s*(import\s+sys\b|from\s+sys\b)', l) for l in lines)
    has_math = any(re.match(r'^\s*(import\s+math\b|from\s+math\b)', l) for l in lines)

    prefix_imports = []
    if 'shutil.' in cleaned and not has_shutil:
        prefix_imports.append('import shutil')
    if 'sys.' in cleaned and not has_sys:
        prefix_imports.append('import sys')
    if 'os.' in cleaned and not has_os:
        prefix_imports.append('import os')
    if 'math.' in cleaned and not has_math:
...
class FileEditTool(Tool):
    ...
    def execute(self, params: Dict[str, Any]) -> str:
        ...
        if path_str.endswith(".py"):
            content = sanitize_python_script(content)
            try:
                ast.parse(content)
